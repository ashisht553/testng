...
function prepareTemp() {
  return gulp.src(join(SHARED_SRC, '**', '*.html'))
...
function processComponentScss() {
  return gulp.src(join(APP_SRC, '**', '*.scss'))
    ...
    .pipe(gulp.dest(isProd ? TMP_DIR + "/" + APP_CLIENT : APP_DEST));
}
 ...
 */
export = () => merge(processComponentStylesheets(), prepareTemplates(),prepareTemp(), processExternalStylesheets());
